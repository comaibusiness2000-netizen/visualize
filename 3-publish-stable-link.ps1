...
$LogPath = Join-Path $ProjectRoot "publish-log.txt"
...
function Stop-WithMessage($Message) {
  ...
  Write-Host "Log salvato in: $LogPath" -ForegroundColor Yellow
  Write-Host ""
...
if ($RepoUrl -notmatch "^https://github\.com/[^/]+/[^/]+\.git$") {
  Stop-WithMessage "URL non valido. Deve somigliare a https://github.com/TUO-UTENTE/visualize-app.git oppure https://github.com/TUO-UTENTE/visualize-app"
}
...
try {
  ...
  & $Git init
  if ($LASTEXITCODE -ne 0) { throw "git init non riuscito" }
  & $Git branch -M main
  ...
  if (![string]::IsNullOrWhiteSpace($Status)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "git commit non riuscito" }
  } else {
  ...
  if ($LASTEXITCODE -ne 0) { throw "git push non riuscito. Controlla login GitHub, URL repo e permessi." }
  ...
  Write-Host "Sara simile a: https://TUO-UTENTE.github.io/NOME-REPO/"
